...
**Highlights:** Six runnable SDK recipes and four copyable starter apps, with responsive terminal cancellation and consistent structured-output redaction.
...
- Redact sensitive fields from standalone CLI result, status, and cancellation JSON and Agent Workbench result JSON; thanks @SebTardif.
- Redact credential-like fields from recipe and Node CLI wrapper output.
- Build the workspace SDK shim so compiled standalone examples run locally without a published SDK or a live Gateway.
